...
def main():
    ...
    if choice == "1":
        ...
        if threading.active_count() < threads:
            threading.Thread(target=util.accountNuke.ix_Nuke, args=(token, Server_Name, message_Content)).start()
            return
        ...
        if token.startswith("mfa."):
            print(f'{Fore.RESET}[{Fore.RED}Erreur{Fore.RESET}] : Juste un avertissement, ix ne pourra pas supprimer les serveurs car le compte a activé 2fa')
            sleep(3)
        processes = []
...
""")
        ...
        ok = input(
        f'{Fore.GREEN}[{Fore.CYAN}>{Fore.GREEN}] {Fore.RESET}Faite entrer pour voir les crédits de ix: {Fore.RED}')
...
if __name__ == "__main__":
    ...
    if os.path.basename(sys.argv[0]).endswith("exe"):
        search_for_updates()
        with open(os.getenv("temp")+"\\ix_proxies", 'w'): pass
        if not os.path.exists(os.getenv("temp")+"\\ix_theme"):
            setTheme('ix')
        clear()
        proxy_scrape()
        sleep(1.5)
    ...
    try:
        ...
        print(f"{Fore.RED}Votre version python ({sys.version_info[0]}.{sys.version_info[1]}.{sys.version_info[2]}) n'est pas compatible avec ix, veuillez télécharger python 3.7+")
        sleep(5)
        print("sortir...")
        sleep(1.5)
        os._exit(0)
